...
const ProductReviewModal = ({ isOpen, onClose, product, user, onReviewSaved }) => {
  ...
  // Pre-fill user details whenever modal opens
  useEffect(() => {
    if (isOpen && user) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosInstance.post("/addreview", formData, {
          ...
          {submitted ? (
            ...
            <div className="flex flex-col items-center text-center py-8 gap-4">
              <div className="w-16 h-16 rounded-full bg-amber-50 flex items-center justify-center">
                <CheckCircle size={36} className="text-amber-500 animate-pulse" />
              </div>
              <h4 className="text-lg font-semibold text-[#1a1a1a] font-poppins">
                Review Submitted!
              </h4>
              <p className="text-sm text-gray-500 font-poppins max-w-xs leading-relaxed">
                Your review was added successfully and will be shown after admin approval.
              </p>
              <button
                onClick={onClose}
                className="mt-2 bg-[#004534] text-white px-8 py-2.5 rounded-full text-sm font-semibold hover:bg-[#003428] transition cursor-pointer font-poppins"
              >
                Done
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label
                    htmlFor="review-name"
                    className="text-sm font-medium text-[#3D3D3D] font-poppins"
                  >
                    Your Name
                  </label>
                  <input
                    ...
                    className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-[#414141] focus:outline-none focus:ring-1 focus:ring-[#004534] transition font-poppins"
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  rows={4}
                  required
                  className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-4 py-3 text-sm text-[#414141] focus:outline-none focus:ring-1 focus:ring-[#004534] transition resize-none font-poppins"
                />
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <label className="flex items-center justify-center gap-2 w-full border-2 border-dashed border-gray-200 rounded-xl px-4 py-4 cursor-pointer hover:border-[#004534] hover:bg-green-50/30 transition bg-[#FAFAFA]">
                ...
                {uploadedImages.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-1">
                    {uploadedImages.map((file, idx) => (
                      <div
                        key={idx}
                        className="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-200 shadow-sm"
                      >
                        <img
